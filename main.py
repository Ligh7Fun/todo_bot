...
RANDOM_TASKS = ['Написать Гвидо письмо',
                'Выучить Python',
...
* show [date]- напечать все задачи на заданную дату
...
def add(message):
    ...
    if len(task) < 3:
        bot.send_message(message.chat.id,
                         'Задачи должны быть больше 3х символов')
    add_todo(date, task)
    bot.send_message(message.chat.id,
                     f'Задача {task} добавлена на дату {date}')
